 ...
 *
 * Löscht einen einzelnen Check-in (DSGVO Art. 17 — Recht auf Löschung).
 * Nur Admins der Firma dürfen löschen. Audit-Log wird geschrieben.
 */
export async function DELETE(
  ...
  try {
    ...
    if (!(await checkAdminRateLimit(ctx.company.id, 'entry-delete', 30, 60_000))) {
      return NextResponse.json({ error: 'Zu viele Anfragen. Bitte warten.' }, { status: 429 })
    }

    const { id } = await params

    // Company-Isolation: nur Einträge der eigenen Firma dürfen gelöscht werden
    ...
    console.error('[entry delete] error:', err instanceof Error ? err.message : String(err))
