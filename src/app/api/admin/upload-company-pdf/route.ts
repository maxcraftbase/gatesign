...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!await checkAdminRateLimit(ctx.company.id, 'upload-company-pdf', 5, 60_000)) {
    ...
    if (!uploadRes.ok) {
      ...
      Sentry.withScope(scope => { scope.setExtras({ companyId: ctx.company.id, fileName, status: uploadRes.status, uploadErr }); Sentry.captureMessage('Company PDF storage upload failed', 'error') })
...
export async function DELETE() {
  try {
    ...
    await fetch(`${supabaseUrl}/rest/v1/app_settings?company_id=eq.${ctx.company.id}&key=eq.company_pdf_url`, {
      method: 'DELETE',
      headers: { apikey: anonKey, Authorization: `Bearer ${ctx.accessToken}` },
    })
